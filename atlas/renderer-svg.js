...
export class SvgAtlasRenderer {
  ...
  render({ activeStates, selectedState = null, transition = {} } = {}) {
    ...
    for (const state of this.geometry.states) {
      ...
      const inFilter = detail.inFilter ?? true;
      const classes = ["atlas-state"];
      if (presentation !== null) classes.push("is-active");
      if (presentation !== null && !inFilter) classes.push("is-filtered-out");
      ...
      path.setAttribute("fill-opacity", state.code === selectedState ? "1" : presentation !== null ? (inFilter ? "0.9" : "0.2") : "0.62");
      path.setAttribute("stroke-opacity", state.code === selectedState ? "1" : presentation !== null ? "0.48" : "0.3");
      path.setAttribute("stroke-width", state.code === selectedState ? "1.5" : "0.7");
      path.setAttribute("stroke", ATLAS_PALETTE.ink);
